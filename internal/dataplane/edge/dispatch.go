...
func (e *Edge) toolHandler(
	...
	return func(ctx context.Context, req *mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		ctx, span := e.startSpan(ctx, "mcp.tools/call",
			...
			observability.AttrAudience.String(av.Audience.Slug),
		...
		if req.Params.RequestState != "" {
			retry, err = e.UnwrapForRetry(
				tool, principal.Subject, av.Audience.Slug, arguments, req.Params.RequestState)
			if err != nil {
		...
		if e.opts.Pipeline != nil {
			...
			case decision.Deferred():
				...
				token, err := e.IssuePluginDeferral(
					tool, principal.Subject, av.Audience.Slug, arguments, decision.RequestState)
				if err != nil {
...
func (e *Edge) catalogMiddleware(view *snapshot.View, av *snapshot.AudienceView) mcp.Middleware {
	return func(next mcp.MethodHandler) mcp.MethodHandler {
		return func(ctx context.Context, method string, req mcp.Request) (mcp.Result, error) {
			...
			// Start from the snapshot's determination, which already accounts
			// for identity-specific policies and identity-dependent catalog
			// plugins.
			identityFiltered := av.IdentityFiltered
			...
			e.log.DebugContext(ctx, "served catalog",
				logging.FieldAudience, av.Audience.Slug,
				logging.FieldPrincipal, principal.Subject,
